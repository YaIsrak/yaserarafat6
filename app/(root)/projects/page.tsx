					...
					<div className='mt-8 divide-y'>
						{data.map((project) => (
							<Link
								href={project.url!}
								key={project._id}
								className='flex justify-between py-7 group'>
								<h2 className='flex-1 text-sm'>{project.title}</h2>
								...
								<div className='flex-1 absolute w-56 left-1/2 -translate-x-1/2'>
									<div className='relative'>
										<Image
											...
											height={500}
											alt={project.title!}
											className='absolute -translate-y-1/2 scale-0 group-hover:scale-100 transition-all duration-300 [transition-timing-function:cubic-bezier(1,0,0,1)]'
										/>
									</div>
								</div>
